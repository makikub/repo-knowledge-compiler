...
- **Page synthesis is a manual LLM step**: `pages/` and `review-aspects/` are never created automatically. None of `ingest`, `ingest-directory`, `ingest-pr-comments`, `lint`, or `compile` write into `pages/` or `review-aspects/`. Synthesis from raw notes into durable pages is an... Source: `.repo-kb/pages/conventions/page-synthesis-workflow.md`
